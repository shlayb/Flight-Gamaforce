...
export default function Home() {
  ...
  // Redirect ke home jika sudah login dan email valid
  useEffect(() => {
    if (user) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (!email || (!email.endsWith('@mail.ugm.ac.id') && !email.endsWith('@ugm.ac.id'))) {
        await auth.signOut();
        throw new Error('Akses ditolak! Hanya akun email UGM yang diizinkan.');
      }
  ...
  return (
    <div className="flex items-center justify-center h-screen ">
      {/* Fixed Background Image with White Overlay for Light Theme */}
      ...
      <div className="relative z-10 w-full max-w-md px-8 sm:px-0 mt-20">
        <div className="glass-card max-md:py-10 p-6 sm:p-8 rounded-3xl relative overflow-hidden">
          {/* Inner glow effect */}
          ...
          <div className="relative z-10 flex flex-col gap-10 ">
            <h1 className="text-3xl sm:text-4xl font-bold mb-6 sm:mb-8 text-center text-biruGelap">Login</h1>
